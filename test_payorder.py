...
class TestAPI():                                        
    ...
    def assertAPI(self, res, esp):                      
        ...
        if int(response['code']) != int(espect['code']):                          #断言失败
            ...
            print('%s Processing: '%self.threadName,        #输出当前哪个线程在处理队列中的哪条数据，设计成队列数据按顺序输出处理
    ...
    def test_payorder(self):                            #执行测试功能的函数                    
        ...
        self.assertAPI(response_dict, self.data)                                      #IOS内购获取订单号接口模块---您看到此信息,代表当行测试数据未通过---  
...
# 定义线程内部run()函数
    def process_data(threadName, q):
        while not exitFlag:
            ...
            if not workQueue.empty():
                ...
                threadtestapi = TestAPI(threadName, url, qdata)         #构造TestAPI对象
    ...
    threadList = list(range(1, 11))          #起多少个线程
    queueLock = threading.Lock()            #创建线程锁
    workQueue = queue.Queue(300)             #创建队列,并设置队列长度，即所需要跑的case数
    threads = []                            #线程列表
    threadID = 1
